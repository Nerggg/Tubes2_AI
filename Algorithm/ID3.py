...
class TreeNode():
    def __init__(self, data=None, feature_idx=None, feature_val=None, prediction_probs=None, feature_importance=None, max_depth=5, min_samples_leaf=1, min_information_gain=1e-7, numb_of_features_splitting="all"):
        self.data = data
        ...
        self.max_depth = max_depth  # Initialize max_depth here
    ...
    def node_def(self):
        """If the node is a leaf returns probs, otherwise it returns split information"""
        if self.feature_idx is None:
            return f"prediction_probs: {self.prediction_probs}"
        else:
            return f"X[{self.feature_idx}] < {self.feature_val}, Feature Importance : {self.feature_importance}"
    ...
    def _find_label_probs(self, data: np.array) -> np.array:
        ...
        # Convert string labels to numerical indices using the label mapping
        labels_as_integers = np.array([label_mapping[label] for label in data[:, -1]])

        # Calculate the total number of labels
        total_labels = len(labels_as_integers)

        # Calculate the ratios (probabilities) for each label
        label_probabilities = np.zeros(len(unique_labels), dtype=float)

        for i, label in enumerate(unique_labels):
            label_index = np.where(labels_as_integers == i)[0]  # Use i instead of label
